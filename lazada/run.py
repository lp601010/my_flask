...
def index():
    ...
    global time_list
    time_list=df.groupby(['业绩年月'])['业绩年月'].first().tolist()
    bar = (
    ...
    bar.overlap(line2)
    bar.overlap(line3)
    return render_template('index.html', myechart=bar.render_embed(),g=g,time_list=[value for value in reversed(time_list)])
...
def show_table(ad_date=mn, host_name='', ad_type=''):
    ...
    # global time_list
    return render_template('month_data.html', df=df, ad_type=ad_type, ad_date=ad_date,time_list=time_list)
